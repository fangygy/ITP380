...
Vector3 SphereCollision::GetCenter()
{
    return mCenter;
}
...
{
	// TODO
    Vector3 mTemp= other->GetCenter()-this->GetCenter();
   
    float distance = mTemp.Length()*mTemp.Length();
    
    float radii=(this->GetActualRadius()+other->GetActualRadius())*(this->GetActualRadius()+other->GetActualRadius());
    if(distance<radii)
    {
            return true;
    }
